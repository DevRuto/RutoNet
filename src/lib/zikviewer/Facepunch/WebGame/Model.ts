...
import { ICompressedMeshData, IMeshData } from './MeshGroup';
import { MeshHandle } from './MeshHandle';
...
export class ModelLoadable extends Model implements ILoadable {
    ...
    loadNext(callback: (requeue: boolean) => void): void {
        ...
        this.http.getJson<IModelInfo>(this.url, info => {
            const materials: Material[] = [];

            for (let i = 0, iEnd = info.materials.length; i < iEnd; ++i) {
                const matUrl = this.http.getAbsUrl(info.materials[i], this.url);
