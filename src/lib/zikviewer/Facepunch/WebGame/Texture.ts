...
export class PixelData<TArray extends Float64Array> implements IPixelData {
  readonly channels: number;
  readonly width: number;
  readonly height: number;
...
export class TextureLoadable extends Texture implements ILoadable {
  ...
  loadNext(callback: (requeue: boolean) => void): void {
      if (this.info == null) {
          this.http.getJson<ITextureInfo>(this.url, info => {
      ...
      const url = this.http.getAbsUrl(element.url, this.url);
      console.log("url test", url);
      this.http.getImage(url, image => {
