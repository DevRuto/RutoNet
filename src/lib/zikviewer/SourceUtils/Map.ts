import { IVector3, Vector3 } from '../Facepunch/Math';
import { Http } from '../Facepunch/Utils';
import {
    CommandBuffer, CommandBufferParameter, ICommandBufferParameterProvider, UniformType
} from '../Facepunch/WebGame/CommandBuffer';
import { DrawList } from '../Facepunch/WebGame/DrawList';
import { MaterialLoadable } from '../Facepunch/WebGame/Material';
import { TextureLoadable, TextureUtils } from '../Facepunch/WebGame/Texture';
import { BspLeaf } from './BspModel';
import { BrushEntity, IBrushEntity } from './Entities/BrushEntity';
import { ISkyCamera, SkyCamera } from './Entities/Camera';
import { Displacement, IDisplacement } from './Entities/Displacement';
import { IKeyframeRope, IMoveRope, KeyframeRope, MoveRope } from './Entities/MoveRope';
import { Entity, IEntity, IEnvFogController, PvsEntity } from './Entities/PvsEntity';
import { IStaticProp, StaticProp } from './Entities/StaticProp';
import { IWorldspawn, Worldspawn } from './Entities/Worldspawn';
...
export class Map implements ICommandBufferParameterProvider {
    ...
    load(url: string, baseurl: string): void {
      this.http.baseUrl = baseurl;
      this.http.getJson<IMap>(url, info => {
          this.onLoad(info);
      });
    }
    ...
    private onLoad(info: IMap): void {
        ...
        for (let i = 0, iEnd = info.entities.length; i < iEnd; ++i) {
            ...
            let inst: Entity = null;
            let pvsInst: PvsEntity = null;
    ...
    getLeafAt(pos: IVector3, callback?: (leaf: BspLeaf) => void): BspLeaf {
