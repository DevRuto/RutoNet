import { ChangedEvent, Event } from './Event';
import { Vector2 } from './Facepunch/Math.js';
import { Key, MouseButton } from './Facepunch/WebGame/Input';
...
import { ReplayFile, TickData } from './ReplayFile';
import { RouteLine } from './RouteLine';
import { CameraMode, MapViewer } from './SourceUtils/MapViewer';
...
export class ReplayViewer extends MapViewer {
	...
	/**
     * Handles info display overlay that shows stats like 
	 * map name, player, mode...
     */
    readonly infoDisplay: InfoDisplay;
	...
	/**
     * If true, makes the replay info display visible.
     * @default `true`
     */
    ...
    /**
     * Event invoked when `showInfoDisplay` changes.
     * 
     * **Available event arguments**:
     * * `showInfoDisplay: boolean` - True if replay info is now visible
     * * `sender: Gokz.ReplayViewer` - This ReplayViewer
     */ 
    readonly showInfoDisplayChanged = new ChangedEvent<boolean, boolean, ReplayViewer>(this);
    ...
    /**
     * Event invoked when `51Mode` changes.
     * 
     * **Available event arguments**:
    ...
    constructor(container: HTMLElement) {
        ...
        this.infoDisplay = new InfoDisplay(this, this.controls.playbackBarElem);
    ...
    protected onChangeReplay(replay: ReplayFile): void {
        ...
        if (this.currentMapName !== replay.mapName) {
            ...
            this.loadMap(`/resources/csgo/maps/${replay.mapName}/index.json?v=${version}`, this.mapBaseUrl);
        }
    }
    ...
    protected onUpdateFrame(dt: number): void {
        ...
        this.showInfoDisplayChanged.update(this.showInfoDisplay);
